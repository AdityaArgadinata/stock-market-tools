...
const InsiderGlobal = ({ token }: InsiderGlobalProps) => {
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [token, page, actionTypeFilter, dateStart, dateEnd]);
    ...
    <div className="h-full flex flex-col gap-4 dark:bg-slate-800 p-4 rounded-md dark:border-slate-700 border">
      <div className="header">
        <h3 className="font-bold text-lg dark:text-slate-100">
          Insider Activity Global
        </h3>
        <p className="text-xs text-gray-500 dark:text-slate-400">
          Major Holder Movements Across All Stocks
        </p>
      </div>
      ...
      <div className="filter-group flex gap-2 flex-wrap">
        <input
          ...
          className="p-2 text-sm border border-gray-300 dark:border-slate-600 rounded-md dark:bg-slate-700 dark:text-slate-100"
        />
        <span className="flex items-center dark:text-slate-300">to</span>
        <input
          type="date"
          value={dateEnd}
          onChange={(e) => {
          ...
          <table className="insider-table w-full border-collapse">
            ...
            <tbody>
              {data.map((item, index) => (
                ...
                >
                  <td className="text-xs p-2 text-left font-semibold dark:text-blue-400">
                    {item.symbol}
                  </td>
                  ...
                  <td className="text-xs p-2 text-left dark:text-slate-200">
                    <div className="flex gap-1 flex-wrap">
                      {item.badges.map((badge, idx) => (
                        <span
                          key={idx}
                          className="inline-flex items-center px-2 py-0.5 text-xs font-semibold rounded-full bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-300 border border-amber-300 dark:border-amber-700"
                        >
